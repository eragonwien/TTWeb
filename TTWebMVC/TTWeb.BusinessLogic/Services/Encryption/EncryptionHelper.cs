...
namespace TTWeb.BusinessLogic.Services.Encryption
{
    public class EncryptionHelper : IEncryptionHelper
    {
        ...
        {
            // Salt and IV is randomly generated each time
            // so that the same Salt and IV values can be used when decrypting.  
            var saltStringBytes = Generate256BitsOfRandomEntropy();
            var ivStringBytes = Generate256BitsOfRandomEntropy();
            var plainTextBytes = Encoding.UTF8.GetBytes(plainText);
            using (var password = new Rfc2898DeriveBytes(_encryptionAppSettings.Secret, saltStringBytes, DerivationIterations))
            {
