...
namespace TTWebCommon.Services
{
    ...
    public class FacebookCredentialService : IFacebookCredentialService
    {
        ...
        public async Task<IEnumerable<FacebookCredential>> GetFacebookCredentialsAsync()
        {
            ...
            var cmdStr =
                @"SELECT appuser_id, fb_credential_id, fb_username, fb_password FROM v_appuser WHERE fb_credential_id is not null ORDER BY fb_credential_id DESC";
        ...
        {
            var cmdStr =
                @"SELECT appuser_id, fb_credential_id, fb_username, fb_password FROM v_appuser WHERE fb_credential_id=@fb_credential_id AND appuser_id=@appuser_id ORDER BY fb_credential_id DESC";
            await using var cmd = await db.CreateCommand(cmdStr);
        ...
        public async Task UpdateFacebookCredentialAsync(FacebookCredential facebookCredential)
        {
            ...
            if (updateCount == 0)
                throw new WebApiNotFoundException("Facebook credentials with id={0} and appuser_id={1} does not exist",
                    facebookCredential.Id, facebookCredential.AppUserId);
        }

        public async Task DeleteFacebookCredentialAsync(int id, int appuserId)
        {
            var cmdStr = "DELETE FROM facebookcredential WHERE id=@id AND appuser_id=@appuser_id";
            await using var cmd = await db.CreateCommand(cmdStr);
